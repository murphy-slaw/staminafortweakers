...
public class SwingingWeaponExhausts implements IRule {
  @Override
  public boolean shouldEnable(StaminaConfig config) {
    return config.depletionPerWeaponSwing > 0;
  }

  @Override
  public boolean test(ServerPlayer player) {
    return ((Exhaustible) player).getSwungWeapon();
  }

  @Override
  public void doResult(StaminaConfig config, ServerPlayer player) {
    ((Exhaustible) player).depleteStamina(config.depletionPerWeaponSwing);
  }
}
